...
impl App {    pub fn new(client_manager: Arc<Mutex<OpcUaClientManager>>) -> Self {
        ...
        subscription_state.select(Some(0));        Self {
            client_manager,
    ...
    async fn run_app(&mut self, terminal: &mut Terminal<CrosstermBackend<Stdout>>) -> Result<()> {
        ...
        // Handle connect screen input first if it's shown
        if self.show_connect_screen {
            if let Some(connection_status) = self.connect_screen.handle_input(key).await? {
        ...
        // Menu bar
        self.menu_renderer.render_menu_bar(f, chunks[0]);        // Main content
        if self.show_connect_screen {
        ...
        // Dialogs (only show if not in connect screen)
        if !self.show_connect_screen {
        ...
        let connection_info = vec![
            ...
            "".to_string(),
            "Commands:".to_string(),
            "  Ctrl+C - Connect to server".to_string(),
            "  F5 - Browse server (when connected)".to_string(),
            "  Alt+X - Exit".to_string(),
        ];

        let info_text = connection_info.join("\n");
        let info_block = Paragraph::new(info_text)
            .block(Block::default()
                .title("OPC UA Client - Main")
                .borders(Borders::ALL))
            .style(Style::default().fg(Color::White));
        f.render_widget(info_block, chunks[0]);

        // Options or help
        let options_text = if self.connection_status == ConnectionStatus::Connected {
            "Connected to OPC UA server. Use the Browse menu to explore the server namespace."
        } else {
            "Not connected. Use Ctrl+C or File menu to connect to an OPC UA server."
        };
